...
/// session_id -> docker container name. One container lives for the whole
/// agent run: every command runs inside it, and it is removed on delete.
...
pub async fn create_session_handler(
    ...
    let container_name = format!("sandbox-session-{session_id}");
    ...
    match Command::new("docker").args(&args).status().await {
        Ok(status) if status.success() => {
            sessions.lock().await.insert(session_id.clone(), container_name);
...
mod tests {
    ...
    async fn session_map_insert_lookup_remove() {
        ...
        sessions
            .lock()
            .await
            .insert("abc".to_string(), "sandbox-session-abc".to_string());
        assert_eq!(container_for(&sessions, "abc").await, Some("sandbox-session-abc".to_string()));
